view: final_table2 {
  derived_table: {
    sql:  SELECT
              ...
              ISV_Site_ID,
              Institution_Name,
              array_to_string(array_agg(Station_name ORDER BY Task_ID),': ') AS Station_name,
              Manufacturer,
              Manufacturer_Model,
              array_to_string(array_agg(Series_Description ORDER BY Task_ID),': ') AS Series_Description,
              array_to_string(array_agg(Series_Datetime ORDER BY Task_ID),': ') AS Series_Datetime,
              array_to_string(array_agg(Datetime_Started ORDER BY Task_ID),': ') AS Datetime_Started,
              array_to_string(array_agg(Datetime_Finished ORDER BY Task_ID),': ') AS Datetime_Finished,
              array_to_string(array_agg(Perf_Acquisition_Type ORDER BY Task_ID),': ') AS Perf_Acquisition_Type,
              array_to_string(array_agg(Perf_Number_Of_Slices ORDER BY Task_ID),': ') AS Perf_Number_Of_Slices,
              array_to_string(array_agg(Perf_Slice_Thickness ORDER BY Task_ID),': ') AS Perf_Slice_Thickness,
              array_to_string(array_agg(Perf_Coverage_z ORDER BY Task_ID),': ') AS Perf_Coverage_z,
              array_to_string(array_agg(Perf_Scan_Duration ORDER BY Task_ID),': ') AS Perf_Scan_Duration,
              array_to_string(array_agg(Perf_Series_Type ORDER BY Task_ID),': ') AS Perf_Series_Type,
              array_to_string(array_agg(Dwi_Number_Of_Slices ORDER BY Task_ID),': ') AS Dwi_Number_Of_Slices,
              array_to_string(array_agg(Dwi_Slice_Thickness ORDER BY Task_ID),': ') AS Dwi_Slice_Thickness,
              array_to_string(array_agg(Dwi_Series_Type ORDER BY Task_ID),': ') AS Dwi_Series_Type,
              array_to_string(array_agg(Ncct_Number_Of_slices ORDER BY Task_ID),': ') AS Ncct_Number_Of_slices,
              array_to_string(array_agg(Ncct_Slice_Thickness ORDER BY Task_ID),': ') AS Ncct_Slice_Thickness,
              array_to_string(array_agg(Ncct_Series_Type ORDER BY Task_ID),': ') AS Ncct_Series_Type,
              array_to_string(array_agg(Cta_Number_Of_Slices ORDER BY Task_ID),': ') AS Cta_Number_Of_Slices,
              array_to_string(array_agg(Cta_Slice_Thickness ORDER BY Task_ID),': ') AS Cta_Slice_Thickness,
              array_to_string(array_agg(Cta_Series_Type ORDER BY Task_ID),': ') AS Cta_Series_Type,
              array_to_string(array_agg(Entry_ID ORDER BY Task_ID),': ') AS Entry_ID,
              array_to_string(array_agg(Task_ID ORDER BY Task_ID),': ') AS Task_ID,
              array_to_string(array_agg(Task_Processing_Type ORDER BY Task_ID),': ') AS Task_Processing_Type,
              array_to_string(array_agg(Task_Result ORDER BY Task_ID),': ') AS Task_Result,
              array_to_string(array_agg(Processing_Time_In_Module ORDER BY Task_ID),': ') AS Processing_Time_In_Module,
              ...
              array_to_string(array_agg(Hemi_Ratio ORDER BY Task_ID),': ') AS Hemi_Ratio,
              array_to_string(array_agg(Aspects_Affected_Side ORDER BY Task_ID),': ') AS Aspects_Affected_Side,
              array_to_string(array_agg(Aspect_Score ORDER BY Task_ID),': ') AS Aspect_Score
              FROM (
