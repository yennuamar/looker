              ...
              SELECT
                    ...
                    FROM (
                      ...
                      SELECT
                        ...
                        ISV_Site_ID,
                        Institution_Name,
                        array_agg(Station_name ORDER BY Task_ID DESC) AS Station_name,
                        Manufacturer,
                        Manufacturer_Model,
                        array_to_string(array_agg(Series_Description ORDER BY Task_ID DESC),', ') AS Series_Description,
                        array_to_string(array_agg(Series_Datetime ORDER BY Task_ID DESC),', ') AS Series_Datetime,
                        array_agg(Datetime_Requested[1] ORDER BY Task_ID DESC) AS Datetime_Requested,
                        array_agg(Datetime_Started[1] ORDER BY Task_ID DESC) AS Datetime_Started,
                        array_agg(Datetime_Finished[1] ORDER BY Task_ID DESC) AS Datetime_Finished,
                        array_agg(Perf_Acquisition_Type[1] ORDER BY Task_ID DESC) AS Perf_Acquisition_Type,
                        array_agg(Perf_Number_Of_Slices[1] ORDER BY Task_ID DESC) AS Perf_Number_Of_Slices,
                        array_agg(Perf_Slice_Thickness[1] ORDER BY Task_ID DESC) AS Perf_Slice_Thickness,
                        array_agg(Perf_Coverage_z[1] ORDER BY Task_ID DESC) AS Perf_Coverage_z,
                        array_agg(Perf_Scan_Duration[1] ORDER BY Task_ID DESC) AS Perf_Scan_Duration,
                        array_agg(Perf_Series_Type[1] ORDER BY Task_ID DESC) AS Perf_Series_Type,
                        array_agg(Dwi_Number_Of_Slices[1] ORDER BY Task_ID DESC) AS Dwi_Number_Of_Slices,
                        array_agg(Dwi_Slice_Thickness[1] ORDER BY Task_ID DESC) AS Dwi_Slice_Thickness,
                        array_agg(Dwi_Series_Type[1] ORDER BY Task_ID DESC) AS Dwi_Series_Type,
                        array_agg(Ncct_Number_Of_slices[1] ORDER BY Task_ID DESC) AS Ncct_Number_Of_slices,
                        array_agg(Ncct_Slice_Thickness[1] ORDER BY Task_ID DESC) AS Ncct_Slice_Thickness,
                        array_agg(Ncct_Series_Type[1] ORDER BY Task_ID DESC) AS Ncct_Series_Type,
                        array_agg(Cta_Number_Of_Slices[1] ORDER BY Task_ID DESC) AS Cta_Number_Of_Slices,
                        array_agg(Cta_Slice_Thickness[1] ORDER BY Task_ID DESC) AS Cta_Slice_Thickness,
                        array_agg(Cta_Series_Type[1] ORDER BY Task_ID DESC) AS Cta_Series_Type,
                        array_to_string(array_agg(Entry_ID[1] ORDER BY Task_ID DESC),', ') AS Entry_ID,
                        Task_ID[1] AS Task_ID,
                        array_agg(Task_Processing_Type[1] ORDER BY Task_ID DESC) AS Task_Processing_Type,
                        array_agg(Task_Result[1] ORDER BY Task_ID DESC) AS Task_Result,
                        array_agg(Processing_Time_In_Module[1] ORDER BY Task_ID DESC) AS Processing_Time_In_Module,
                        array_agg(Total_Processing_Time_Since_Delivery[1] ORDER BY Task_ID DESC) AS Total_Processing_Time_Since_Delivery,
                        array_agg(Username[1] ORDER BY Task_ID DESC) AS Username,
                        array_agg(Number_Of_Slabs[1] ORDER BY Task_ID DESC) AS Number_Of_Slabs,
                        array_agg(Parameter_Name[1] ORDER BY Task_ID DESC) AS Parameter_Name,
                        array_agg(Threshold[1] ORDER BY Task_ID DESC) AS Threshold,
                        array_agg(Volume[1] ORDER BY Task_ID DESC) AS Volume,
                        array_agg(Cta_Affected_Side[1] ORDER BY Task_ID DESC) AS Cta_Affected_Side,
                        array_agg(Hemi_Ratio[1] ORDER BY Task_ID DESC) AS Hemi_Ratio,
                        array_agg(Aspects_Affected_Side[1] ORDER BY Task_ID) AS Aspects_Affected_Side,
                        array_agg(Aspect_Score[1] ORDER BY Task_ID) AS Aspect_Score,
                        array_agg(Task_Result_Code[1] ORDER BY Task_ID) AS Task_Result_Code
                        FROM (
