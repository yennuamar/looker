view: main_table_1 {
  derived_table: {
    sql: SELECT  rapid_patient_id,
        ...
        manufacturer,
        manufacturer_model,
        array_agg(datetime_started_time order by task_id) AS datetime_started_time,
        array_agg(datetime_finished_time order by task_id) AS datetime_finished_time,
        array_agg(series_description order by task_id) AS series_description,
        array_agg(series_datetime_time order by task_id) AS series_datetime_time,
        ...
        array_agg(n_slabs order by task_id) AS n_slabs,
        ...
        array_agg(cta1_affected_side order by task_id) AS cta1_affected_side,
        array_agg(hem_ratio order by task_id) AS hem_ratio,
