view: main_table_1 {
  derived_table: {
    sql: SELECT  rapid_patient_id,
        ...
        manufacturer,
        manufacturer_model,
        array_agg(datetime_started_time) AS datetime_started_time,
        array_agg(datetime_finished_time) AS datetime_finished_time,
        array_agg(series_description) AS series_description,
        array_agg(series_datetime_timed) AS series_datetime_time,
        array_agg(entry_id) AS entry_id,
        array_agg(task_id) AS task_id,
        array_agg(task_processing_type) AS task_processing_type,
        array_agg(task_result) AS task_result,
        array_agg(processing_time_in_module) AS processing_time_in_module,
        array_agg(total_processing_time_since_delivery) AS total_processing_time_since_delivery,
        array_agg(username) AS username,
        array_agg(n_slabs) AS n_slabs,
        array_agg(parameter_name) AS parameter_name,
        array_agg(threshold) AS threshold,
        array_agg(volume) AS volume,
        array_agg(cta1_affected_side) AS cta1_affected_side,
        array_agg(hem_ratio) AS hem_ratio,
        array_agg(aspects_affected_side) AS aspects_affected_side,
        array_agg(aspect_score) AS aspect_score
      FROM (
